...
class Rack_LoadedRack(Task):
    ...
    def _create_scene(self) -> None:
        ...
        self.sim.create_table(length=1.6, width=1.2, height=0.5, x_offset=-0.3)
        ...
        self.sim._create_geometry(
            ...
            geom_type=self.sim.physics_client.GEOM_MESH,
            mass= 0.0,
            position=self.vial_goal_positions[5],
            ghost=True,
            visual_kwargs=visual_kwargs,
            collision_kwargs=collision_kwargs,
        )
        # -------------------------------------- Vials in Target Rack ---------------------------------------------
        cup_visual = os.getcwd()+'/Objects/Vial.obj'
        cup_collision = os.getcwd()+'/Objects/Vial_vhacd.obj'
        mesh_scale = [0.03] * 3
        visual_kwargs = {
        ...
        self.sim._create_geometry(
            ...
            geom_type=self.sim.physics_client.GEOM_MESH,
            mass= 0.1,
            position=self.vial_goal_positions[1],
            ghost=False,
            visual_kwargs=visual_kwargs,
            collision_kwargs=collision_kwargs,
        )
        self.sim._create_geometry(
            ...
            geom_type=self.sim.physics_client.GEOM_MESH,
            mass= 0.1,
            position=self.vial_goal_positions[2],
            ghost=False,
            visual_kwargs=visual_kwargs,
            collision_kwargs=collision_kwargs,
        )
        ...
        self.sim._create_geometry(
            ...
            geom_type=self.sim.physics_client.GEOM_MESH,
            mass= 0.1,
            position=self.vial_goal_positions[3],
            ghost=False,
            visual_kwargs=visual_kwargs,
            collision_kwargs=collision_kwargs,
        )
        self.sim._create_geometry(
            ...
            geom_type=self.sim.physics_client.GEOM_MESH,
            mass= 0.1,
            position=self.vial_goal_positions[4],
            ghost=False,
            visual_kwargs=visual_kwargs,
            collision_kwargs=collision_kwargs,
        )
    ...
    def reset(self) -> None:
        ...
        self.Insert_Flag = False
        rand1 = np.array([random.random()*0.03, random.random()*0.03, 0])
        rand2 = np.array([random.random()*0.03, random.random()*0.03, 0])
        ...
        target_positions = [self.vial_goal_positions[self.targrand - 1] + rand2] + \
        [self.vial_goal_positions[i] + rand2 for i in range(6) if i != self.targrand - 1]
        
        target_names = ["vial_to_place"] + [f"vial_target{i + 1}" for i in range(5)]
        for target_name, target_position in zip(target_names, target_positions):
    ...
    def is_success(self, achieved_goal: np.ndarray, desired_goal: np.ndarray) -> np.ndarray:
        ...
        vial_bodies = ['vial_target1', 'vial_target2', 'vial_target3', 'vial_target4', 'vial_target5']
        
        
        vial_contact_counts = [len(p.getContactPoints(bodyA=self.sim._bodies_idx[vial], bodyB=self.sim._bodies_idx['vial_initial'],
