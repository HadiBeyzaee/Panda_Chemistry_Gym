...
import panda_chemistry_gym.assets
...
class PyBullet:
    ...
    def __init__(self, render: bool = False, n_substeps: int = 50, background_color: Optional[np.ndarray] = None) -> None:
        background_color = background_color if background_color is not None else np.array([25.0, 154.0, 45.0])
    ...
    def create_box(
        ...
        if texture is not None:
            texture_path = os.path.join(panda_gym2.assets.get_data_path(), texture)
