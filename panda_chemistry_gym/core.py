...
class PyBulletRobot(ABC):
    ...
    def get_camera_data(self):
        ...
        return self.sim.get_camera_data(self.body_name)
...
class RobotTaskEnv(gym_robotics.GoalEnv):
    ...
    def __init__(self, robot: PyBulletRobot, task: Task) -> None:
        ...
        #     dict(
        #         observation=gym.spaces.Box(-1.0, 1.0, shape=observation_shape, dtype=np.float32),
        #         desired_goal=gym.spaces.Box(-1.0, 1.0, shape=achieved_goal_shape, dtype=np.float32),
        #         achieved_goal=gym.spaces.Box(-1.0, 1.0, shape=desired_goal_shape, dtype=np.float32),
        #     )
        # )

        self.observation_space = gym.spaces.Dict(
            dict(
                observation=gym.spaces.Box(-200.0, 200.0, shape=observation_shape, dtype=np.float32),
                desired_goal=gym.spaces.Box(-200.0, 200.0, shape=achieved_goal_shape, dtype=np.float32),
                achieved_goal=gym.spaces.Box(-200.0, 200.0, shape=desired_goal_shape, dtype=np.float32),
            )
        )

        self.action_space = gym.spaces.Box(-1.0, 1.0, self.robot.action_space.shape, dtype=np.float32)
